	...
	for (let attempt = 1; attempt <= 10; attempt++) {
		try {
			...
			while (Date.now() - start < 20) {
				// Synchronous callers deliberately avoid an async trust API.
			}
		}
	}
	throw (lastError as Error) ?? new Error("Failed to acquire trust store lock");
}
...
export function hasProjectTrustInputs(cwd: string): boolean {
	...
	if (hasProjectConfigDir(currentDir)) return true;
	while (true) {
...
export class ProjectTrustStore {
	...
	get(cwd: string): ProjectTrustDecision {
		return withTrustFileLock(this.trustPath, () => {
			const value = readTrustFile(this.trustPath)[normalizeCwd(cwd)];
