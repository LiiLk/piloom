...
vi.mock("../src/package-manager-cli.js", () => ({
	...
	isSelfUpdateSource: (source: string) => source === "self" || source === "pi" || source === "piloom",
}));
...
describe("public command routing", () => {
	...
	it("rejects extra attach operands", async () => {
		...
		expect(console.error).toHaveBeenCalledWith(expect.stringContaining("piloom attach <agent>"));
	});
	...
	it("gives legacy update targets explicit migration guidance", async () => {
		for (const target of ["self", "--self", "piloom"]) {
			await handlePublicCommand(["update", target]);
		}

		expect(mocks.packageCommands).toEqual([]);
		expect(console.error).toHaveBeenCalledWith(expect.stringContaining('Use "piloom update [--force]"'));
	});
	...
	it("directs legacy package-update forms to the package command", async () => {
		...
		expect(mocks.packageCommands).toEqual([]);
		expect(console.error).toHaveBeenCalledWith(expect.stringContaining('Use "piloom package update [source]"'));
	});
	...
	it("rejects self-update aliases on the package update path", async () => {
		for (const source of ["self", "pi", "piloom"]) {
			await handlePublicCommand(["package", "update", source]);
		}

		expect(mocks.packageCommands).toEqual([]);
		expect(console.error).toHaveBeenCalledWith(expect.stringContaining('Use "piloom update"'));
	});
	...
	it("directs package uninstall to package remove", async () => {
		...
		expect(mocks.packageCommands).toEqual([]);
		expect(console.error).toHaveBeenCalledWith(expect.stringContaining('Use "piloom package remove"'));
	...
	it("rejects operands for package list", async () => {
		...
		expect(mocks.packageCommands).toEqual([]);
		expect(console.error).toHaveBeenCalledWith(expect.stringContaining("piloom package list"));
	});
	...
	it("rejects the old daemon hierarchy with migration guidance", async () => {
		...
		expect(process.exitCode).toBe(1);
		expect(console.error).toHaveBeenCalledWith(expect.stringContaining('Run "piloom help"'));
	});

	it("shows migration guidance when help targets removed commands", async () => {
		const cases: Array<[path: string[], hint: string]> = [
			[["daemon"], 'Run "piloom help"'],
			[["install"], 'Use "piloom package install"'],
			[["remove"], 'Use "piloom package remove"'],
			[["uninstall"], 'Use "piloom package remove"'],
			[["manage"], 'Use "piloom agents"'],
			[["app", "update"], 'Use "piloom update"'],
		];
	...
	it("shows command help when options precede the help flag", async () => {
		...
		expect(console.log).toHaveBeenNthCalledWith(1, expect.stringContaining("piloom list [--all] [--json]"));
		expect(console.log).toHaveBeenNthCalledWith(2, expect.stringContaining("piloom doctor [--fix] [--json]"));
		expect(console.log).toHaveBeenNthCalledWith(3, expect.stringContaining("piloom package install <source>"));
