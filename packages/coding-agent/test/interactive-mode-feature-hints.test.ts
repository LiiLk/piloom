...
describe("feature hint deck", () => {
	...
	it("covers PiLoom workflows with capability-focused copy", () => {
		const deck = new FeatureHintDeck(() => 0);
		...
		expect(textById.get("subagents")).toBe("PiLoom can delegate tasks to subagents and run them in parallel.");
